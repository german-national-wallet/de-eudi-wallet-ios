...
final class AnalyticsControllerImpl: AnalyticsController {
  ...
  func startTrace(name: String, initialAttributes: [String: String]) {
    analyticsConfig?.analyticsProviders.forEach { (_, provider) in
      provider.startTrace(name: name, initialAttributes: initialAttributes)
    }
  }

  @discardableResult
  func endTrace(finalAttributes: [String: String], errorDescription: String?) -> String? {
    analyticsConfig?.analyticsProviders.values.compactMap {
      $0.endTrace(finalAttributes: finalAttributes, errorDescription: errorDescription)
    }.first
  }

  var currentTraceID: String? {
    analyticsConfig?.analyticsProviders.values.compactMap(\.currentTraceID).first
  }
}
