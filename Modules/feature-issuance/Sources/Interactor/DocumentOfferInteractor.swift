...
final class DocumentOfferInteractorImpl: DocumentOfferInteractor {
  ...
  init(
    ...
    func issueDocuments(
      ...
      do {
        ...
        if issuedDocuments.isEmpty {
          ...
          await replacePreExistingDuplicates(newDocumentIds: documentIdentifiers)
    ...
    func resumeDynamicIssuance(
      ...
      do {
        ...
        if doc.status == .issued {
          await replacePreExistingDuplicates(newDocumentIds: [doc.id])
    ...
    private func replacePreExistingDuplicates(newDocumentIds: [String]) async {
      let newDocumentIdSet = Set(newDocumentIds)
      let newlyIssued = walletController.fetchDocuments(with: newDocumentIds)
      guard !newlyIssued.isEmpty else { return }

      let allIssued = walletController.fetchIssuedDocuments()

      for newDoc in newlyIssued {
        guard let issuer = newDoc.credentialIssuerIdentifier else { continue }
        let type = newDoc.docType
        guard !type.isEmpty else { continue }

        let typeIdentifier = DocumentTypeIdentifier(rawValue: type)
        guard typeIdentifier != .mDocPid, typeIdentifier != .sdJwtPid else { continue }

        let duplicates = allIssued.filter {
          $0.credentialIssuerIdentifier == issuer
            && $0.docType == type
            && !newDocumentIdSet.contains($0.id)
        }
        for duplicate in duplicates {
          try? await walletController.deleteDocument(with: duplicate.id, status: .issued)
        }
      }
    }
