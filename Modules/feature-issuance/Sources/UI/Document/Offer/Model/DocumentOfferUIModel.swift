...
public extension DocumentOfferUIModel {
  /// Background color for the offered credential, derived from the offer's display
  /// metadata (falls back to a deterministic color), mirroring the issued-document card.
  var backgroundColor: Color {
    guard let docOffer = docOffers.first else {
      return DSColor.colorPID
    }
    return DSColor.getBackgroundColorForCredential(
      display: docOffer.displayMetadata,
      id: docOffer.credentialConfigurationIdentifier
    )
  }

  var credentialLogoUrl: String? {
    docOffers.first?.credentialLogoUrl
  }
}
