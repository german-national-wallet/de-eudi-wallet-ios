...
final class DocumentTabViewModel<Router: RouterHost>: ViewModel<Router, DocumentTabState> {
  ...
  func loadMoreCredentials() async {
    guard let privateKey = secureEnclaveController.retrievePrivateKey(with: .credentialPrivKey) else {
        return
    }
    do {
      _ = try await credentialsInteractor.getCredentialsWithRefreshToken([CredentialType(documentType: nil, scope: ScopeValue.pid.rawValue, identifier: configLogic.pidMsoMdocConfigId, docDataFormat: .cbor)], privateKey: privateKey)
      fetch()
    } catch {
