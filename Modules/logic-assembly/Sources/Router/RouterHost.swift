...
final class RouterHostImpl: RouterHost {
  ...
  init(
    ...
    #else
    self.pilot = UIPilot(initial: .featureStartupModule(.startup), debug: false)
    #endif
    self.logger = logger
  }
  ...
  public func resetNavigation(to route: AppRoute) {
    guard canNavigate(block: self.resetNavigation(to: route)) else { return }
    lockNavigation()
    pilot.popTo(.featureStartupModule(.startup), inclusive: false, animated: false)
    if getCurrentScreen() != route {
      pilot.push(route)
    }
    onNavigationFollowUp(with: route)
  }
