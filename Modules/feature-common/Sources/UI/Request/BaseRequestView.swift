...
public struct BaseRequestView<Router: RouterHost>: View {
  ...
  public init(
    with router: Router,
    viewModel: BaseRequestViewModel<Router>
  ) {
...
@MainActor
@ViewBuilder
private func confirmationScreen(
  viewState: RequestViewState,
  getScreenRect: CGRect,
  continue: @escaping () -> Void
) -> some View {
  VStack(alignment: .leading, spacing: DSStyle.Spacers.SPACING_EXTRA_LARGE) {
    DSStyle.Spacers.VSpacer.extraLarge()
    
    Image("linear-determinate", bundle: .main)
      .resizable()
      .frame(width: getScreenRect.width - 32, height: 10)
    
    VStack(alignment: .leading, spacing: DSStyle.Spacers.SPACING_EXTRA_LARGE) {
      // TODO: Remove hardcoded RP Name
      Text(.presentationConfirmationTitle([""]))
    ...
    Button(action: {
        // TODO: To be completed after the presentation fix
    }, label: {
        Text(.viewDetails([""]))
    ...
    HStack {
        Button(action: {
            ...
            Text(.presentationConfirmationCancelButton)
                .font(DSTypography.Label.large)
                .fontWeight(DSStyle.FontWeight.medium_500)
                .foregroundColor(DSColor.primary)
        })
        ...
        Button(action: {
            ...
            Text(.presentationConfirmationContinueButton)
                .font(DSTypography.Label.large)
                .fontWeight(DSStyle.FontWeight.medium_500)
