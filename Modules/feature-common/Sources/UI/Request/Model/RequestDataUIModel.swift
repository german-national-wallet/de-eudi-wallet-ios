  ...
  func prepareRequest() -> RequestItemsWrapper {
    ...
    let requestConvertible = models
      ...
      .reduce(into: RequestItemsWrapper()) { partialResult, claim in
        ...
        let path = requestedElementPath(
          type: claim.domainModel?.type,
          fullPath: claim.domainModel?.path
        )
...
public extension Array where Element == DocElements {
  func toUiModels(with walletKitController: WalletKitController) -> [RequestDataUiModel] {
    self.compactMap { element in
      ...
      case .sdJwt(let doc):
        doc.sdJwtElements.compactMap { $0.requestedDocClaim }
      }
...
// Maps a requested claim's full element path to the path sent in the presentation request.
// - mdoc: the namespace is path[0], so the element identifier is path[1].
// - sdjwt: the full nested path with empty segments (array wildcards) dropped, e.g.
//   ["address", "locality"] stays ["address", "locality"] rather than collapsing to ["locality"].
// Kept internal (not closure-local) so it can be unit-tested as a pure transformation.
func requestedElementPath(type: DocumentElementType?, fullPath: [String]?) -> [String] {
  switch type {
  case .mdoc:
    guard let fullPath, fullPath.count > 1 else { return [] }
    return [fullPath[1]]
  case .sdjwt:
    guard let fullPath else { return [] }
    return fullPath.compactMap { $0.isEmpty ? nil : $0 }
  default:
    return []
  }
}

// Internal (not private) so the children-recursion behavior can be unit-tested.
extension SdJwtElement {
  var requestedDocClaim: DocClaim? {
    guard var claim = docClaim else { return nil }
    if let nestedElements, !nestedElements.isEmpty {
      claim.children = nestedElements.compactMap { $0.requestedDocClaim }
    }
    return claim
  }
}
